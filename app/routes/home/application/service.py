...
from app.routes.home.application.core.calculator import HomeCalculator
...
class HomeService:
    ...
    async def fetch_line_queue(self, scenario_id: str | None):
        ...
        calculator = HomeCalculator(pax_df, facility_info)
    ...
    async def fetch_summary(
        ...
        pax_df = await self.home_repo.download_simulation_parquet_from_s3(scenario_id)
        facility_info = await self.home_repo.download_json_from_s3(scenario_id)
        calculator = HomeCalculator(pax_df, facility_info, calculate_type, percentile)

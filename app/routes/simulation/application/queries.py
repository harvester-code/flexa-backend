...
SELECT F.ACTUAL_BLOCK_TIME,
       ...
       F.FLIGHT_DATE_UTC,
       F.FLIGHT_ID,
       F.OPERATING_CARRIER_IATA || F.FLIGHT_NUMBER       AS FLIGHT_NUMBER,
       CASE
       ...
       SPLIT_PART(A1.TIMEZONE_REGION_NAME, '/', 1)       AS REGION_NAME,
       F.TAIL_NUMBER,
       F.TOTAL_SEAT_COUNT,
...
WHERE (F.ARRIVAL_AIRPORT_ID = %(airport)s OR F.ARRIVAL_AIRPORT_IATA = %(airport)s)
  AND F.SCHEDULED_GATE_ARRIVAL_LOCAL::date = %(date)s
...
SELECT F.ACTUAL_BLOCK_TIME,
       ...
       F.OPERATING_CARRIER_ID,
       F.OPERATING_CARRIER_NAME,
       SPLIT_PART(A2.TIMEZONE_REGION_NAME, '/', 1)       AS REGION_NAME,
       F.TAIL_NUMBER,
       F.TOTAL_SEAT_COUNT,
       ...
       F.DEPARTURE_GATE,
       COALESCE(F.DEPARTURE_TERMINAL, 'unknown') AS DEPARTURE_TERMINAL,
...
WHERE (F.DEPARTURE_AIRPORT_ID = %s OR F.DEPARTURE_AIRPORT_IATA = %s)
  AND F.SCHEDULED_GATE_DEPARTURE_LOCAL::date = %s
  AND F.TOTAL_SEAT_COUNT > 0
"""
...
WHERE (F.DEPARTURE_AIRPORT_ID = %(airport)s OR F.DEPARTURE_AIRPORT_IATA = %(airport)s)
  AND F.SCHEDULED_GATE_DEPARTURE_LOCAL::date = %(date)s
  AND F.TOTAL_SEAT_COUNT > 0
"""
...
SELECT NULL AS ACTUAL_BLOCK_TIME,
       ...
       NULL AS FLIGHT_ID,
       S.OPERATING_CARRIER_IATA || S.FLIGHT_NUMBER       AS FLIGHT_NUMBER,
       CASE
           WHEN A1.COUNTRY_CODE = A2.COUNTRY_CODE THEN 'Domestic'
           ELSE 'International'
       END AS FLIGHT_TYPE,
       ...
       COALESCE(S.DEPARTURE_TERMINAL, 'unknown') AS DEPARTURE_TERMINAL,
...
WHERE (S.DEPARTURE_STATION_CODE_ID = %s OR S.DEPARTURE_STATION_CODE_IATA = %s)
  AND S.PASSENGER_DEPARTURE_TIME_LOCAL::date = %s
  AND S.TOTAL_SEATS > 0
...
WHERE (S.DEPARTURE_STATION_CODE_ID = %(airport)s OR S.DEPARTURE_STATION_CODE_IATA = %(airport)s)
  AND S.PASSENGER_DEPARTURE_TIME_LOCAL::date = %(date)s
  AND S.TOTAL_SEATS > 0
  AND S.IS_CODESHARE = 0
"""

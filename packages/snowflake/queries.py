...
#
# 항공사명 / 공항 도시·국가·지역 / 항공기 기종명 → 모두 enrichment에서
# oag_ref.xlsx (Airline Code / Airport Code / Aircraft Code 시트) 기반으로 후처리
#
# Snowflake는 별칭을 대문자로 반환하므로, 소문자 유지를 위해
# 모든 별칭을 쌍따옴표(")로 감싸야 함
#
# Named parameters: %(flight_date)s, %(airport)s → 호출 시 dict로 전달
# CTE로 출발편(DEPAPT) + 도착편(ARRAPT)을 UNION ALL 조회 후 최종 SELECT 1회 수행
...
SELECT
    ...
    s.CARRIER || CASE
        WHEN LENGTH(LTRIM(s.FLTNO::VARCHAR, '0')) >= 3 THEN LTRIM(s.FLTNO::VARCHAR, '0')
        ELSE LPAD(LTRIM(s.FLTNO::VARCHAR, '0'), 3, '0')
    END                                                                         AS "flight_number",
    s.CARRIER                                                                   AS "marketing_carrier_iata",
    s.CARRIER                                                                   AS "operating_carrier_iata",
    NULL                                                                        AS "operating_carrier_name",
